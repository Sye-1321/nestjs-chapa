const sensitiveKeys =
  /authorization|secret|signature|token|password|email|phone|account|first.?name|last.?name|body|checkout.?url/i;
...
export function redactSensitive(value: unknown, knownSecrets: readonly string[] = []): unknown {
  if (typeof value === 'string') {
    ...
    const resemblesSensitiveData =
      /bearer\s+\S+|\b(?:secret|signature|token|password)\b|[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}|(?:\+?\d[\s().-]*){7,}/i.test(
        value
      );
