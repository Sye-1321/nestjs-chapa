...
export class ReferencesResource implements ChapaReferences {
  ...
  generate(options: GenerateReferenceOptions = {}): string {
    ...
    if (prefix !== undefined && !componentPattern.test(prefix))
      issues.push({ path: ['prefix'], message: 'prefix must use the reference alphabet' });
    if (options.separator !== undefined && prefix === undefined)
      issues.push({ path: ['separator'], message: 'separator requires a prefix' });
    if (options.separator !== undefined && separator !== '' && !componentPattern.test(separator))
      issues.push({ path: ['separator'], message: 'separator must use the reference alphabet' });
    if (!Number.isInteger(size) || size <= 0)
      issues.push({ path: ['size'], message: 'size must be a positive integer' });
