...
const inputSchema = z
  .object({
    ...
    customization: z
      .object({ title: usableString.optional(), description: usableString.optional(), logo: usableString.optional() })
      .strict()
      .optional(),
...
function validationError(message: string, result: z.ZodError): ChapaValidationError {
  return new ChapaValidationError(
    message,
    result.issues.map(({ path, message: issueMessage }) => ({
...
function responseError(
  operation: 'payments.initialize' | 'payments.verify',
  ...
  return new ChapaResponseError({
    ...
    message,
    operation,
    method: operation === 'payments.verify' ? 'GET' : 'POST',
...
function jsonSafetyIssue(
  ...
  try {
    ...
    if (prototype !== Object.prototype && prototype !== null)
      return { path, message: 'JSON records must be plain objects' };
...
export class PaymentsResource implements ChapaPayments {
  ...
  async initialize(
    ...
    if (parsed.data.callbackUrl) {
      ...
      if (
        url.protocol !== 'https:' &&
        !(url.protocol === 'http:' && local && this.#configuration.allowInsecureTestUrls)
      ) {
        throw new ChapaValidationError('Invalid payment initialization input', [
          { path: ['callbackUrl'], message: 'callbackUrl must use HTTPS outside explicitly enabled local tests' }
        ]);
      }
    }
    const wire = {
    ...
    if (!status || !checkoutUrl)
      throw responseError('payments.initialize', result, 'Chapa initialization response is missing required fields');
    try {
      new URL(checkoutUrl);
    } catch {
      throw responseError(
        'payments.initialize',
        result,
        'Chapa initialization response contains an unusable checkout URL'
      );
    }
  ...
  async verify(txRef: string, options?: ChapaSafeReadRequestOptions): Promise<VerifyPaymentResult> {
    if (!txRefPattern.test(txRef))
      throw new ChapaValidationError('Invalid transaction reference', [
        { path: ['txRef'], message: 'txRef must match the SDK grammar' }
      ]);
    const result = await this.#executor.execute({
      policy: { operation: 'payments.verify', method: 'GET', path: `/transaction/verify/${txRef}`, retry: 'safe-read' },
      ...(options ? { options } : {})
    });
    const raw = objectValue(result.data);
    const data = raw ? objectValue(raw.data) : undefined;
    ...
    if (!providerStatus)
      throw responseError('payments.verify', result, 'Chapa verification response is missing transaction status');
    const known = new Set<PaymentStatus>(['success', 'pending', 'failed', 'cancelled', 'refunded', 'reversed']);
    const status: PaymentStatus = known.has(providerStatus as PaymentStatus)
      ? (providerStatus as PaymentStatus)
      : 'unknown';
    const amount =
      data && typeof data.amount === 'string' && decimalStringPattern.test(data.amount) ? data.amount : undefined;
    const charge =
      data && typeof data.charge === 'string' && decimalStringPattern.test(data.charge) ? data.charge : undefined;
  ...
  async cancel(txRef: string, options?: ChapaMutationRequestOptions): Promise<CancelPaymentResult> {
    ...
    const result = await this.#executor.execute({
      policy: { operation: 'payments.cancel', method: 'PUT', path: `/transaction/cancel/${txRef}`, retry: 'never' },
      ...(options ? { options } : {})
    });
    const raw = objectValue(result.data);
    const message = raw ? optionalString(raw, 'message') : undefined;
