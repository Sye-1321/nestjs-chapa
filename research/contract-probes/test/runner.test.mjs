...
describe('M0.5-C Interactive Runner Tests', () => {
  ...
  test('1. actual module import/evaluation makes zero fetch calls', async () => {
    let fetchCalled = 0;
  ...
  test('2. startup performs zero network and TEST MODE required', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
    const fetchFunc = async () => { fetchCalled++; };

    const runnerPromise = runInteractiveSession({ stdin: h.stdin, stdout: h.stdout, fetchFunc, env: { CHAPA_SECRET_KEY: 'test' } });
  ...
  test('3. scenario display performs zero network', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
    const fetchFunc = async () => { fetchCalled++; };
  ...
  test('4. missing credential => zero fetch', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
    const fetchFunc = async () => { fetchCalled++; };

    const runnerPromise = runInteractiveSession({ stdin: h.stdin, stdout: h.stdout, fetchFunc, env: {} });
    await h.waitFor('TEST MODE');
    h.write('TEST MODE\n');
    await h.waitFor('Select scenario');
  ...
  test('5. AUTHORIZE required for every exact request, random text cannot authorize', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
    const fetchFunc = async () => { fetchCalled++; };
  ...
  test('6. pre-buffered/multiple AUTHORIZE lines cannot authorize a later request', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
  ...
  test('7. one authorization => exactly one fetch, authorization cannot carry between scenarios', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
    const fetchFunc = async (url) => {
      fetchCalled++;
  ...
  test('8. initialize => exactly one POST, verify => exactly one GET, no runner retry', async () => {
    const h = createInteractiveHarness();
    const calls = [];
  ...
  test('9. initialization timeout preserves tx_ref, no replay, no auto verify', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
    const fetchFunc = async (url, opts) => {
      fetchCalled++;
  ...
  test('10. verification transport failure uses verification-specific behavior', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
    const fetchFunc = async (url, opts) => {
      fetchCalled++;
  ...
  test('11. secret absent from success and error output, checkout URL/token absent from all output', async () => {
    const h = createInteractiveHarness();
    const fetchFunc = async (url) => {
      const resp = {
        message: 'Hosted Link',
    ...
    const runnerPromise = runInteractiveSession({ stdin: h.stdin, stdout: h.stdout, fetchFunc, env: { CHAPA_SECRET_KEY: 'my-super-secret-key-12345' } });
    await h.waitFor('TEST MODE');
    h.write('TEST MODE\n');
    await h.waitFor('Select scenario');
  ...
  test('12. generated refs <= 50 and match conservative grammar', async () => {
    const h = createInteractiveHarness();
    const fetchFunc = async (url) => {
      return { url: url.toString(), arrayBuffer: async () => new ArrayBuffer(0), headers: new Headers(), status: 200 };
    };
  ...
  test('13. duplicate uses exact accepted payload, can execute once only, persistent guard blocks across sessions', async () => {
    ...
    if (fs.existsSync(duplicateGuardPath)) {
      fs.unlinkSync(duplicateGuardPath);
    }
    ...
    const runnerPromise2 = runInteractiveSession({ stdin: h2.stdin, stdout: h2.stdout, fetchFunc, env: { CHAPA_SECRET_KEY: 'test' } });
  ...
  test('14. unknown verification generates a fresh reference in accepted observed-style grammar <= 50 chars', async () => {
    const h = createInteractiveHarness();
    let fetchUrl = null;
    const fetchFunc = async (url) => {
  ...
  test('15. runner cannot silently/batch execute scenarios, returning to menu performs no additional fetch', async () => {
    const h = createInteractiveHarness();
    let fetchCalled = 0;
    const fetchFunc = async (url) => {
      fetchCalled++;
  ...
  test('16. fake provider execution causes raw capture only under .raw', async () => {
    const h = createInteractiveHarness();
    let captureUrl = null;
    const fetchFunc = async (url) => {
  ...
  test('17. provider message hardening adversarial tests (malicious object keys)', async () => {
    const h = createInteractiveHarness();
    const fetchFunc = async (url) => {
      const resp = {
        message: {
