...
- M1-2 completed the OQ-07 proof recorded in `docs/contracts/2026-08-26-m1-oq-07-build-tool-proof.md`: `tsc` 5.9.3 and stable `tsdown` 0.22.14 both passed actual-tarball, Node 22.0.0/24.0.0, Nest 10/11, ESM/CJS, typing, boundary, externalization, determinism, and API-extraction checks. `tsc` is selected because its non-bundling output is equally correct with fewer transformation layers, transparent output, and declaration maps; `tsdown` supplied no requirement-level advantage and is not selected. This is an M1-2 selection record only and does not begin M1-3.
- M1-2 maintainer review approved the evidence and resolved OQ-07 on 2026-08-26. M1-3 removed `tsdown` from the active toolchain and froze the selected `tsc` 5.9.3 architecture: 17 validated files under `dist/esm/**` and `dist/cjs/**`, declarations and declaration maps in both trees, a deterministic CommonJS subtree marker, and conditional `import`/`require` exports with nested type targets for only `.` and `./testing`.
- Microsoft API Extractor 7.59.0 uses committed canonical root and `./testing` reports from the ESM declarations and compares the CJS reports byte-for-byte for one equivalent public API. The baselines are intentionally empty and fail on accidental public exports or dependency-type leakage.
- The npm `files` allowlist remains `dist`, `README.md`, `LICENSE`, `CHANGELOG.md`, and `SECURITY.md`; package metadata and actual-tarball checks enforce that no source, research/evidence, plan, workflow, script, test, lockfile, workspace/config, proof, cache, or environment path ships.
- The pnpm workspace file now states an active 1,440-minute minimum-release-age policy. Exact Nest 11.2.3 proof-fixture exceptions are temporary because those reviewed versions were younger than the policy window on 2026-08-26; remove the exceptions after both releases age past the window. Dependency changes remain lockfile-reviewed.
- `reflect-metadata` remains dev-only. The production runtime dependency is only Zod, and Nest 10/11 remain peers. M1-3 adds no public symbols or M2-M5 behavior.
- M1-4 established 31 permanent Node built-in foundation tests covering identity, exact exports and output, API baselines/parity, public-type leakage, deterministic clean builds, stale-output removal, actual tarball contents, map path safety, inert imports, and research separation. The permanent pack check validates and then removes an actual 22-file tarball.
- The permanent selected-build consumer matrix is centralized in `test/consumers/matrix.mjs`: Nest 10.4.22 ESM/Node 22, Nest 10.4.22 CJS/Node 24, Nest 11.2.3 ESM/Node 24, and Nest 11.2.3 CJS/Node 22. Every isolated consumer installs the generated tarball, runs strict NodeNext and bundler typing, executes decorator/runtime checks, blocks deep imports, and has no Chapa credential or provider call. An opt-in `--floors` harness mode reproduces exact Node 22.0.0/24.0.0 Docker validation; focused final-tarball ESM/22.0.0 and CJS/24.0.0 sanity checks passed.
