...
const body = Buffer.from('{"event":"charge.success","status":"success","tx_ref":"tx_1","amount":"001.20","currency":"ETB","extra":{"safe":true}}');
...
test('X1 accepts exact lowercase and uppercase HMAC and configured/explicit secret precedence', () => {
  ...
  assert.equal(webhooks.verify({ rawBody: body, headers: { 'x-chapa-signature': generateChapaTestSignature({ rawBody: body, secret: 'override' }) }, secret: 'override' }).event.event, 'charge.success');
});
...
test('X1 rejects missing, wrong, mutated, malformed, whitespace, and reconstructed bytes', () => {
  ...
  const mutated = Buffer.from(body); mutated[0] ^= 1;
...
test('C1 is optional secondary consistency only and never rescues X1', () => {
  ...
  assert.equal(verify(body, { 'x-chapa-signature': signature, 'chapa-signature': secondary }).event.event, 'charge.success');
  for (const value of ['bad', '0'.repeat(64)]) assert.throws(() => verify(body, { 'x-chapa-signature': signature, 'chapa-signature': value }), errors.ChapaWebhookSignatureError);
  assert.throws(() => verify(body, { 'x-chapa-signature': '0'.repeat(64), 'chapa-signature': secondary }), errors.ChapaWebhookSignatureError);
});
...
test('header lookup is case-insensitive and fails closed on arrays and ambiguity', () => {
  ...
  assert.throws(() => verify(body, { 'x-chapa-signature': signature, 'X-Chapa-Signature': signature }), errors.ChapaWebhookSignatureError);
});
...
test('event normalization preserves known, unknown, exact string money, and raw provider object', () => {
  const known = verify();
  assert.deepEqual({ event: known.event.event, status: known.event.status, txRef: known.event.txRef, amount: known.event.amount, currency: known.event.currency }, { event: 'charge.success', status: 'success', txRef: 'tx_1', amount: '001.20', currency: 'ETB' });
  assert.equal(known.event.raw.extra.safe, true);
  for (const payload of [
    { event: 'charge.success', status: 'failed' },
    { event: 'future.event', status: 'new', trx_ref: 'bad', reference: 'bad', ref_id: 'bad', payment_reference: 'bad', amount: 12.5 }
  ]) {
    ...
    if (payload.event === 'future.event') { assert.equal(result.event.txRef, undefined); assert.equal(result.event.amount, undefined); }
  }
  const noEvent = Buffer.from('{"status":"success"}');
  assert.throws(() => verify(noEvent, { 'x-chapa-signature': generateChapaTestSignature({ rawBody: noEvent, secret }) }), errors.ChapaResponseError);
});
...
test('signature errors redact secrets, signatures, raw bodies and caller bytes are unchanged', () => {
  ...
  assert.throws(() => verify(caller, { 'x-chapa-signature': '0'.repeat(64) }), (error) => {
...
test('missing configured and explicit webhook secret is a configuration error distinct from API key', () => {
  ...
  assert.throws(() => resource.verify({ rawBody: body, headers: { 'x-chapa-signature': signature } }), errors.ChapaConfigurationError);
});
