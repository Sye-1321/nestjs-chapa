...
const banksFixture = JSON.parse(await readFile(resolve(root, 'test/fixtures/chapa/banks.test-mode.success.json'), 'utf8'));
const currenciesFixture = JSON.parse(await readFile(resolve(root, 'test/fixtures/chapa/currencies.test-mode.success.json'), 'utf8'));

function reply(status, value) {
  return { status, headers: { 'content-type': 'application/json' }, body: new TextEncoder().encode(JSON.stringify(value)), durationMs: 1 };
}
...
function harness(responses) {
  ...
  return {
    client: new ChapaClient({ secretKey: 'synthetic-secret', transport, retry: { maxSafeRetries: 1, baseDelayMs: 0, maxDelayMs: 0, jitter: false } }),
    requests
  };
}
...
test('banks map only evidence-backed fields and preserve numeric id', async () => {
  ...
  assert.deepEqual(result.banks[0], {
    id: 656, name: 'Awash Bank', slug: 'awash_bank', swift: 'AWINETAA', accountLength: 15, currency: 'ETB', raw: banksFixture.data.data[0]
  });
...
test('bank unknown and integer/null flag fields remain raw without public boolean projections', async () => {
  ...
  for (const key of ['isMobileMoney', 'isActive', 'isRtgs', 'active', 'is24hrs', 'canProcessPayouts', 'canProcessPayments']) assert.equal(bank[key], undefined);
...
test('bank optional fields require usable wire types', async () => {
  const body = { data: [{ name: 'Minimal', id: null, slug: 1, swift: '', acct_length: '10', currency: null, unknown: 'retained' }] };
...
test('banks safe-read retries eligible failure once', async () => {
  ...
  assert.equal(requests.length, 2); assert.equal(result.response.attempts, 2);
});
...
test('currency membership is not hard-coded and numeric provider code remains numeric', async () => {
  const result = await harness([reply(200, { currency_code: [987], currency_name: ['FUTURE'] })]).client.metadata.listCurrencies();
  assert.deepEqual(result.currencies[0], { providerCode: 987, name: 'FUTURE', raw: { currency_code: 987, currency_name: 'FUTURE' } });
...
test('currency codes must be finite numbers and names usable strings', async () => {
  for (const body of [
    { currency_code: ['1'], currency_name: ['ETB'] }, { currency_code: [Number.NaN], currency_name: ['ETB'] },
    { currency_code: [1], currency_name: [2] }, { currency_code: [1], currency_name: [''] }
  ]) await assert.rejects(harness([reply(200, body)]).client.metadata.listCurrencies(), errors.ChapaResponseError);
});
...
test('metadata performs no hidden caching', async () => {
  ...
  await client.metadata.listBanks(); await client.metadata.listBanks();
  assert.equal(requests.length, 2);
});

test('metadata 404 remains generic ChapaApiError', async () => {
  await assert.rejects(harness([reply(404, { message: 'missing' })]).client.metadata.listBanks(), (error) => error.constructor === errors.ChapaApiError);
});
...
test('metadata implementation stays internal while public contracts are exported', async () => {
  ...
  for (const contract of ['ChapaMetadata', 'ChapaBank', 'ListBanksResult', 'ChapaCurrency', 'ListCurrenciesResult']) assert.match(report, new RegExp(contract));
