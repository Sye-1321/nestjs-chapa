...
const safePolicy = Object.freeze({
  operation: 'metadata.listBanks',
  method: 'GET',
  path: '/banks',
  retry: 'safe-read'
});
const mutationPolicy = Object.freeze({
  operation: 'payments.initialize',
  method: 'POST',
  path: '/transaction/initialize',
  retry: 'never'
});
const cancelPolicy = Object.freeze({
  operation: 'payments.cancel',
  method: 'PUT',
  path: '/transaction/cancel/synthetic_ref',
  retry: 'never'
});
...
test('configuration validates secure URLs, ranges, and transport shape', () => {
  assert.throws(
    () => configModule.resolveChapaConfiguration({ secretKey: 'x', baseUrl: 'http://example.com' }),
  ...
  assert.throws(
    () => configModule.resolveChapaConfiguration({ secretKey: 'x', timeoutMs: Infinity }),
    errors.ChapaConfigurationError
  );
  assert.throws(
    () => configModule.resolveChapaConfiguration({ secretKey: 'x', transport: {} }),
    errors.ChapaConfigurationError
  );
});
...
test('configuration inspection and errors redact secrets', () => {
  ...
  const error = new errors.ChapaApiError({
    ...
    retryable: false,
    raw: { authorization: `Bearer ${secret}`, body: secret, nested: { secretKey: secret } }
  });
...
test('executor constructs exact authentication and safe headers without correlation header', async () => {
  ...
  assert.equal(
    Object.keys(captured.headers).some((key) => key.includes('correlation')),
    false
  );
});
...
test('mutation HTTP failure makes one attempt and rejects runtime retry controls', async () => {
  ...
  await assert.rejects(
    executor.execute({ policy: mutationPolicy, body: '{}', options: { maxRetries: 1 } }),
    errors.ChapaValidationError
  );
  assert.equal(calls, 1);
});
...
test('safe read retries one eligible network failure and accounts attempts', async () => {
  ...
  const { executor } = executorWith({
    send: async () => {
      calls += 1;
      if (calls === 1) throw new Error('socket');
      return response();
    }
  });
...
test('SDK timeout maps distinctly and never duplicates mutation', async () => {
  ...
  await assert.rejects(
    executor.execute({ policy: mutationPolicy, body: '{}', options: { timeoutMs: 10 } }),
    errors.ChapaTimeoutError
  );
  assert.equal(calls, 1);
});
...
test('caller abort maps distinctly and stops retry progression', async () => {
  ...
  await assert.rejects(
    executor.execute({ policy: safePolicy, options: { signal: caller.signal } }),
    errors.ChapaAbortError
  );
  assert.equal(calls, 1);
});
...
test('HTTP status maps to typed API errors with safe metadata', async () => {
  const cases = [
    [401, errors.ChapaAuthenticationError],
    [403, errors.ChapaPermissionError],
    [429, errors.ChapaRateLimitError],
    [500, errors.ChapaApiError]
  ];
  for (const [status, Type] of cases) {
    const { executor } = executorWith(
      { send: async () => response(status, { status: 'failed', message: 'safe' }) },
      { retry: { maxSafeRetries: 0 } }
    );
    await assert.rejects(
      executor.execute({ policy: safePolicy }),
      (error) => error instanceof Type && error.httpStatus === status && error.attempts === 1
    );
  }
});
...
test('observability receives allowlisted metadata only and failures are best-effort', async () => {
  ...
  const { executor } = executorWith(
    { send: async () => response() },
    { logging: { enabled: true, level: 'debug' }, logger, hooks }
  );
  ...
  assert.deepEqual(Object.keys(contexts[1]).sort(), [
    'attempts',
    'correlationId',
    'durationMs',
    'endpoint',
...
test('configured logging level filters lower-level observations while hooks remain independent', async () => {
  for (const [level, expected] of [
    ...
    const { executor } = executorWith(
      { send: async () => response() },
      { logging: { enabled: true, level }, logger, hooks }
    );
  ...
  const { executor } = executorWith(
    { send: async () => response() },
    { logging: { enabled: false, level: 'debug' }, logger }
  );
  await executor.execute({ policy: safePolicy, options: { maxRetries: 0 } });
