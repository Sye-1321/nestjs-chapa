...
function reply(status, value, durationMs = 1) {
  return { status, headers: { 'content-type': 'application/json' }, body: new TextEncoder().encode(typeof value === 'string' ? value : JSON.stringify(value)), durationMs };
}
...
function harness(responses, options = {}) {
  ...
  const client = new ChapaClient({ secretKey: 'synthetic-secret', transport, retry: { maxSafeRetries: 1, baseDelayMs: 0, maxDelayMs: 0, jitter: false }, ...options });
...
test('initialize rejects invalid amount forms before transport without Number conversion', async () => {
  for (const amount of ['0', '0.0', '0.00', '-1', '+1', '1e2', ' 1', '1,000', '01', '1.234', '', 10]) {
    const { client, requests } = harness([initSuccess]);
    await assert.rejects(client.payments.initialize({ ...minimal, amount }), errors.ChapaValidationError); assert.equal(requests.length, 0);
  }
  const huge = '9'.repeat(200) + '.99'; const { client, requests } = harness([initSuccess]);
  await client.payments.initialize({ ...minimal, amount: huge }); assert.equal(JSON.parse(requests[0].body).amount, huge);
});

test('initialize accepts txRef lengths 1 and 50 and rejects 51', async () => {
  for (const txRef of ['a', 'A'.repeat(50)]) await harness([initSuccess]).client.payments.initialize({ ...minimal, txRef });
  const { client, requests } = harness([initSuccess]);
  await assert.rejects(client.payments.initialize({ ...minimal, txRef: 'A'.repeat(51) }), errors.ChapaValidationError); assert.equal(requests.length, 0);
});

test('initialize rejects invalid email, phone, unknown subaccounts, and callback policy', async () => {
  for (const input of [{ ...minimal, email: 'invalid' }, { ...minimal, phoneNumber: '123' }, { ...minimal, subaccounts: [] }, { ...minimal, callbackUrl: 'http://merchant.example.test/callback' }]) {
    const { client, requests } = harness([initSuccess]); await assert.rejects(client.payments.initialize(input), errors.ChapaValidationError); assert.equal(requests.length, 0);
  }
  await harness([initSuccess], { allowInsecureTestUrls: true }).client.payments.initialize({ ...minimal, callbackUrl: 'http://127.0.0.1/callback' });
});
...
test('initialize timeout and network uncertainty make exactly one POST', async () => {
  const timeoutHarness = harness([(request) => new Promise((_resolve, reject) => request.signal.addEventListener('abort', () => reject(new DOMException('aborted', 'AbortError')), { once: true }))]);
  await assert.rejects(timeoutHarness.client.payments.initialize(minimal, { timeoutMs: 10 }), errors.ChapaTimeoutError); assert.equal(timeoutHarness.requests.length, 1);
  const network = harness([new Error('socket')]); await assert.rejects(network.client.payments.initialize(minimal), errors.ChapaNetworkError); assert.equal(network.requests.length, 1);
});

test('initialize rejects mutation maxRetries supplied through untyped JavaScript', async () => {
  const { client, requests } = harness([initSuccess]); await assert.rejects(client.payments.initialize(minimal, { maxRetries: 1 }), errors.ChapaValidationError); assert.equal(requests.length, 0);
});
...
test('verify omits numeric money, preserves valid lexical strings, and echoes caller txRef', async () => {
  const numeric = await harness([reply(200, { data: { status: 'success', amount: 10.10, charge: 0.3, tx_ref: 'provider_alias' } })]).client.payments.verify('caller_ref');
  assert.equal(numeric.amount, undefined); assert.equal(numeric.charge, undefined); assert.equal(numeric.txRef, 'caller_ref');
  const lexical = await harness([reply(200, { data: { status: 'success', amount: '10.10', charge: '0.30' } })]).client.payments.verify('caller_ref');
  assert.equal(lexical.amount, '10.10'); assert.equal(lexical.charge, '0.30');
});
